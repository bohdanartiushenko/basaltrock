...
The test uses knowledge base files from `src/test/resources/basaltrock/docker/data/`. During test execution:
1. A temporary Docker build context is created in `/tmp`
2. Docker files are copied from `src/main/resources/basaltrock/docker/`
3. Test data files are copied from `src/test/resources/basaltrock/docker/data/`
...
public class MyTest {
    ...
    static TempComposeBasaltrockContainer ragContainer = 
        new TempComposeBasaltrockContainer(
            "src/main/resources/basaltrock/docker",
            "src/test/resources/basaltrock/docker/data");
