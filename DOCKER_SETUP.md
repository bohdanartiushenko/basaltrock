...
- **Scanner service**: Uploads embeddings to OpenSearch index
...
### Build both services:
```bash
cd src/main/resources/opensearch/docker
...
| `COSINE_LIMIT` | `0` | Min similarity score for retrieval (0.0-1.0, AWS Bedrock compatible) |
...
### Scanner fails with "No .txt or .html files found"
...
private static final ComposeContainer compose =
    new ComposeContainer(new File("src/main/resources/opensearch/docker/docker-compose-test.yml"))
...
4. Scanner will regenerate embeddings cache automatically

## Performance Considerations

- **Embeddings cache**: Cached after first run, only regenerated when data changes
- **Startup time**: Scanner must complete before API starts (~1-5 minutes depending on data size)
